...
//	화면에서 사용자 아이디 전달받음
	public boolean checkId(String userIdentification) {
		boolean check = false;
//		String query = "SELECT USER_ID FROM TBL_USER WHERE USER_IDENTIFICATION = \""+ userIdentification + "\"";
...
//	회원탈퇴, UPDATE(일반 회원:0, 탈퇴 회원:1)
	public void unregister() {
		...
		try {
			...
			System.out.println("unregister() SQL문 오류");
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
...
//	아이디 찾기(휴대폰 번호)
	public String selectId(String userPhone) {
		...
		try {
			...
			System.out.println("selectId(String) SQL문 오류");
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
...
//	비밀번호 변경(로그인 후 현재 비밀번호, 새로운 비밀번호)
	public void changePassword(String nowPassword, String newPassword) {
		...
		try {
			...
			preparedStatement.setLong(2, userId);
			preparedStatement.setString(3, nowPassword);
			preparedStatement.executeUpdate();
//			변경된 행의 수
		} catch (SQLException e) {
...
//		int number = 0;
//		String query = "SELECT COUNT(USER_RECOMMENDER_ID) " 
//				+"FROM TBL_USER " 
//				+"WHERE USER_RECOMMENDER_ID = (SELECT USER_IDENTIFICATION  FROM TBL_USER WHERE USER_ID =?)";
//		connection = DBConnecter.getConnection();
//		try {
//			preparedStatement = connection.prepareStatement(query);
//			preparedStatement.setLong(1, userId);
...
//		} catch (SQLException e) {
//			System.out.println("recommendNumber() SQL문 오류");
//			e.printStackTrace();
//		} catch (Exception e) {
//			e.printStackTrace();
//		} finally {
...
//	//내방법
//	public ArrayList<String> foundRecommendId() {
//		ArrayList<String> idList = new ArrayList<String>();
//		boolean check = false;
//		String query = "SELECT USER_RECOMMENDER_ID " 
//				+"FROM TBL_USER " 
//				+"WHERE USER_RECOMMENDER_ID = (SELECT USER_IDENTIFICATION  FROM TBL_USER WHERE USER_ID =?)";
//		connection = DBConnecter.getConnection();
//		try {
...
//			}
//		} catch (SQLException e) {
//			System.out.println("foundRecommendId() SQL문 오류");
//			e.printStackTrace();
//		} catch (Exception e) {
//			e.printStackTrace();
//		} finally {
...
//	//내방법
//	public String foundMyRecommendId() {
...
//			resultSet = preparedStatement.executeQuery();
//			resultSet.next();
//			recommendId = resultSet.getString(1);
//		} catch (SQLException e) {
//			System.out.println("foundMyRecommendId() SQL문 오류");
//			e.printStackTrace();
//		} catch (Exception e) {
//			e.printStackTrace();
//		} finally {
